...
void UpdateAndRender(World* world) {
  ...
  if (ImGui::DockBuilderGetNode(dockspace_id) == nullptr) {
    ...
    ImGuiID dockIDLeft = ImGui::DockBuilderSplitNode(
        dockMainID, ImGuiDir_Left, 0.20f, nullptr, &dockMainID);
    ImGuiID dockIDRight = ImGui::DockBuilderSplitNode(
        dockMainID, ImGuiDir_Right, 0.20f, nullptr, &dockMainID);
  ...
  ImGui::DockSpace(dockspace_id, ImVec2(0.0f, 0.0f), dockspace_flags);
  {
    ImGui::Begin("Viewport");
    {
      ...
      if (size != g_LastViewportSize) {
        ...
        cameraComponent.proj = glm::perspective(
            60.0f * beard::math::DegToRad, (f32)size.x / size.y, 0.1f, 5000.0f);
    ...
    ImGui::Begin("Entities");
    {
      ...
      for (auto entity : view) {
        if (ImGui::Selectable(view.get<NameComponent>(entity).name.c_str(),
    ...
    ImGui::Begin("Light");
    {
      ...
      // ImGui::RadioButton("None", &renderer.backgroundType,
      // (int)BackgroundType::None); ImGui::RadioButton("Cubemap",
      // &renderer.backgroundType, (int)BackgroundType::Cubemap);
      // ImGui::RadioButton("Irradiance", &renderer.backgroundType,
      // (int)BackgroundType::Irradiance); ImGui::RadioButton("Radiance",
      // &renderer.backgroundType, (int)BackgroundType::Radiance);
    ...
    ImGui::Begin("Renderer");
    {
      ...
      ImGui::Begin("Post-Process");
      {
        ...
        ImGui::DragFloat("Threshold", &renderer.bloom.threshold, 0.1f, 0.0f,
                         10.0f, "%.1f");
        ImGui::DragFloat("Knee", &renderer.bloom.knee, 0.01f, 0.0f, 10.0f,
                         "%.2f");
        ImGui::DragFloat("Intensity", &renderer.bloom.intensity, 0.1f, 0.0f,
                         10.0f, "%.1f");
    ...
    if (ImGui::CollapsingHeader("Material", ImGuiTreeNodeFlags_DefaultOpen)) {
      if (g_SelectedEntity != entt::null) {
        ...
        if (material->hasAlbedoTexture) {
          if (ImGui::ImageButton((void*)(intptr_t)material->albedoTexture,
        ...
        if (material->hasRoughnessTexture) {
          if (ImGui::ImageButton((void*)(intptr_t)material->roughnessTexture,
                                 ImVec2(64, 64), ImVec2(0, 1), ImVec2(1, 0))) {
        ...
        if (material->hasMetallicTexture) {
          if (ImGui::ImageButton((void*)(intptr_t)material->metallicTexture,
                                 ImVec2(64, 64), ImVec2(0, 1), ImVec2(1, 0))) {
        ...
        ImGui::Checkbox("Metallic - Roughness texture",
                        &material->hasMetallicRoughnessTexture);
        if (material->hasMetallicRoughnessTexture) {
          if (ImGui::ImageButton(
                  (void*)(intptr_t)material->metallicRoughnessTexture,
                  ImVec2(64, 64), ImVec2(0, 1), ImVec2(1, 0))) {
        ...
        if (material->hasEmissiveTexture) {
          if (ImGui::ImageButton((void*)(intptr_t)material->emissiveTexture,
                                 ImVec2(64, 64), ImVec2(0, 1), ImVec2(1, 0))) {
        ...
        if (material->hasEmissive || material->hasEmissiveTexture) {
          ImGui::SliderFloat("Emissive factor", &material->emissiveFactor, 0.0f,
                             10.0f);
        }

        ImGui::Checkbox("Normal map", &material->hasNormalMap);
        if (material->hasNormalMap) {
          if (ImGui::ImageButton((void*)(intptr_t)material->normalMap,
                                 ImVec2(64, 64), ImVec2(0, 1), ImVec2(1, 0))) {
        ...
        if (material->hasAmbientOcclusionMap) {
          if (ImGui::ImageButton((void*)(intptr_t)material->ambientOcclusionMap,
                                 ImVec2(64, 64), ImVec2(0, 1), ImVec2(1, 0))) {
    ...
    ImGui::Begin("Stats");
    {
      ...
      ImGui::Text("\t\t\tBloom prefilter: %.3lfms",
                  stats->frame.bloomPrefilter);
      ImGui::Text("\t\t\tBloom downsample: %.3lfms",
                  stats->frame.bloomDownsample);
      ImGui::Text("\t\t\tBloom upsample first pass: %.3lfms",
                  stats->frame.bloomUpsampleFirst);
      ImGui::Text("\t\t\tBloom upsample: %.3lfms", stats->frame.bloomUpsample);
      ImGui::Text("\t\tFinal compositing: %.3lfms",
                  stats->frame.finalCompositing);
      ...
      for (i32 i = 0; i < renderableView.size(); ++i) {
        ...
        const auto& mesh =
            world->GetEntity(entity).GetComponent<MeshComponent>();

        i64 vertexCount = mesh.vertexCount;
        i64 triangleCount = mesh.indexCount / 3;
        ImGui::Text("\tModel %d has %lld vertices and %lld triangles", i,
                    vertexCount, triangleCount);
        vertexTotal += vertexCount;
        triangleTotal += triangleCount;
      }
      ImGui::Text("Totalizing %lld vertices and %lld triangles", vertexTotal,
                  triangleTotal);
    }
    ImGui::End();
...
bool IsCursorInViewport(f64 x, f64 y) {
  return (x >= g_ViewportX && x <= (g_ViewportX + g_ViewportW)) &&
         (y >= g_ViewportY && y <= (g_ViewportY + g_ViewportH));
}
}  // namespace UI
