...
inline std::string TexturePath(const char* texture,
...
inline std::shared_ptr<Material> ProcessMaterial(
    aiMaterial* inputMaterial,
    const aiScene* scene,
    const std::filesystem::path& path) {
  auto material = std::make_shared<Material>(inputMaterial->GetName().C_Str(),
  ...
  if (AI_SUCCESS ==
      inputMaterial->GetTexture(AI_MATKEY_BASE_COLOR_TEXTURE, &albedoTexture)) {
    material->hasAlbedoTexture = true;
    material->albedoTexture =
        LoadTexture(TexturePath(albedoTexture.C_Str(), path));
  }

  aiString metallicTexture;
  if (AI_SUCCESS ==
      inputMaterial->GetTexture(AI_MATKEY_METALLIC_TEXTURE, &metallicTexture)) {
    material->hasMetallicTexture = true;
    material->metallicTexture =
        LoadTexture(TexturePath(metallicTexture.C_Str(), path));
  }

  aiString roughnessTexture;
  if (AI_SUCCESS == inputMaterial->GetTexture(AI_MATKEY_ROUGHNESS_TEXTURE,
                                              &roughnessTexture)) {
    material->hasRoughnessTexture = true;
    material->roughnessTexture =
        LoadTexture(TexturePath(roughnessTexture.C_Str(), path));
  }

  aiString metallicRoughnessTexture;
  if (AI_SUCCESS ==
      inputMaterial->GetTexture(
          AI_MATKEY_GLTF_PBRMETALLICROUGHNESS_METALLICROUGHNESS_TEXTURE,
          &metallicRoughnessTexture)) {
    material->hasMetallicRoughnessTexture = true;
    material->metallicRoughnessTexture =
        LoadTexture(TexturePath(metallicRoughnessTexture.C_Str(), path));
  }

  aiColor3D emissiveColor;
  if (AI_SUCCESS ==
      inputMaterial->Get(AI_MATKEY_COLOR_EMISSIVE, emissiveColor)) {
  ...
  if (AI_SUCCESS ==
      inputMaterial->GetTexture(aiTextureType_EMISSIVE, 0, &emissiveTexture)) {
    material->hasEmissiveTexture = true;
    material->emissiveTexture =
        LoadTexture(TexturePath(emissiveTexture.C_Str(), path));
  }

  aiString occlusionTexture;
  if (AI_SUCCESS ==
      inputMaterial->GetTexture(aiTextureType_LIGHTMAP, 0, &occlusionTexture)) {
    material->hasAmbientOcclusionMap = true;
    material->ambientOcclusionMap =
        LoadTexture(TexturePath(occlusionTexture.C_Str(), path));
  }
...
bool LoadAssimpScene(const char* filename, World* world) {
  ...
  const u32 importerFlags = aiProcess_Triangulate | aiProcess_GenSmoothNormals |
                            aiProcess_CalcTangentSpace |
                            aiProcess_OptimizeMeshes;
  const aiScene* scene = importer.ReadFile(filename, importerFlags);

  if ((nullptr == scene) ||
      (0 != (scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE)) ||
