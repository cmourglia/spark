...
void LoadNode(u32 nodeIndex,
  ...
  if (!node.matrix.empty()) {
    ...
    if (!node.translation.empty()) {
      auto translation = glm::vec3{node.translation[0], node.translation[1],
                                   node.translation[2]};
  ...
  // TODO: This is already broken. We cannot really flatten the scenegraph as we
  // might have animations that touch a parent node of a mesh.
