...
template <typename T>
void BindBuffer(const beard::array<T>& buffer,
                i32 vao,
                i32 vbo,
                i32 bindingPoint,
                i32 offset,
                ...
                GLenum dataType) {
  if (!buffer.is_empty()) {
    glNamedBufferSubData(vbo, offset, buffer.data_size(), buffer.data());
...
RenderMesh::RenderMesh(const MeshComponent& mesh) {
  ...
  m_IndexType = mesh.indexType == IndexType::Unsigned16 ? GL_UNSIGNED_SHORT
                                                        : GL_UNSIGNED_INT;
  ...
  u32 accessFlags = GL_MAP_WRITE_BIT | GL_MAP_READ_BIT | GL_MAP_PERSISTENT_BIT |
                    GL_MAP_COHERENT_BIT;
  // u32 accessFlags = 0;
  ...
  // Upload data
  BindBuffer(mesh.positions, m_VAO, m_Buffer, 0, positionOffset, 3, GL_FLOAT);
  BindBuffer(mesh.normals, m_VAO, m_Buffer, 1, normalOffset, 3, GL_FLOAT);
  BindBuffer(mesh.texcoords, m_VAO, m_Buffer, 2, texcoordOffset, 2, GL_FLOAT);
  BindBuffer(mesh.weights, m_VAO, m_Buffer, 3, weightsOffset, 4, GL_FLOAT);
  BindBuffer(mesh.bones, m_VAO, m_Buffer, 4, bonesOffset, 4, GL_FLOAT);

  // Get positions and normal buffer
  glm::vec3* buffer = (glm::vec3*)glMapNamedBufferRange(
      m_Buffer, positionOffset, positionSize + normalSize, accessFlags);
...
void RenderMesh::DrawInstanced(u32 instanceCount) const {
  glBindVertexArray(m_VAO);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_Buffer);
  glDrawElementsInstanced(GL_TRIANGLES, m_IndexCount, m_IndexType, nullptr,
                          instanceCount);
}
