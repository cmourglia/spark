...
RenderMesh::RenderMesh(Mesh mesh) : m_Mesh(std::move(mesh)) {
  i32 indexOffset = 0;
  ...
  // u32 accessFlags = GL_MAP_WRITE_BIT | GL_MAP_READ_BIT |
  ...
  // glm::vec3* buffer = (glm::vec3*)glMapNamedBufferRange(m_Buffer,
  // positionOffset, positionSize + normalSize, accessFlags); m_PositionBuffer
  // = buffer; m_NormalBuffer    = buffer + positionSize;
}
...
void RenderMesh::DrawInstanced(u32 instanceCount) const {
  glBindVertexArray(m_VAO);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_Buffer);
  glDrawElementsInstanced(GL_TRIANGLES, m_Mesh.indexCount, m_IndexType, nullptr,
                          instanceCount);
}
