...
void Renderer::Initialize(const glm::vec2& initialSize) {
  ...
  Program::MakeCompute("equirectangularToCubemap",
                       "equirectangular_to_cubemap.comp.glsl");
  Program::MakeCompute("prefilterEnvmap", "prefilter.comp.glsl");
  Program::MakeCompute("irradiance", "irradiance.comp.glsl");

  m_backgroundProgram = Program::MakeRender(
      "background", "background.vert.glsl", "background.frag.glsl");
...
void Renderer::Resize(const glm::vec2& newSize) {
  if (m_framebufferSize != newSize) {
    ...
    glTextureStorage2DMultisample(msaaRenderTexture, 4, GL_RGBA32F, newSize.x,
                                  newSize.y, GL_TRUE);
    glNamedFramebufferTexture(m_msaaFB, GL_COLOR_ATTACHMENT0, msaaRenderTexture,
                              0);

    // Create MSAA DS rendertarget and attach it to FBO
    glCreateRenderbuffers(1, &msaaDepthRenderBuffer);
    glNamedRenderbufferStorageMultisample(
        msaaDepthRenderBuffer, 4, GL_DEPTH24_STENCIL8, newSize.x, newSize.y);
    glNamedFramebufferRenderbuffer(m_msaaFB, GL_DEPTH_STENCIL_ATTACHMENT,
                                   GL_RENDERBUFFER, msaaDepthRenderBuffer);

    if (glCheckNamedFramebufferStatus(m_msaaFB, GL_FRAMEBUFFER) !=
    ...
    glNamedFramebufferTexture(m_resolveFB, GL_COLOR_ATTACHMENT0, resolveTexture,
                              0);
    glTextureParameteri(resolveTexture, GL_TEXTURE_MIN_FILTER,
                        GL_LINEAR_MIPMAP_LINEAR);
    glTextureParameteri(resolveTexture, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    if (glCheckNamedFramebufferStatus(m_resolveFB, GL_FRAMEBUFFER) !=
        GL_FRAMEBUFFER_COMPLETE) {
    ...
    m_bloomSize.x += m_bloomComputeWorkGroupSize -
                     ((i32)m_bloomSize.x % m_bloomComputeWorkGroupSize);
    m_bloomSize.y += m_bloomComputeWorkGroupSize -
                     ((i32)m_bloomSize.y % m_bloomComputeWorkGroupSize);
    ...
    glTextureStorage2D(bloomTextures[0], mipCount, GL_RGBA32F, m_bloomSize.x,
                       m_bloomSize.y);
    glTextureStorage2D(bloomTextures[1], mipCount, GL_RGBA32F, m_bloomSize.x,
                       m_bloomSize.y);
    glTextureStorage2D(bloomTextures[2], mipCount, GL_RGBA32F, m_bloomSize.x,
                       m_bloomSize.y);

    glTextureParameteri(bloomTextures[0], GL_TEXTURE_MIN_FILTER,
                        GL_LINEAR_MIPMAP_LINEAR);
    glTextureParameteri(bloomTextures[0], GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTextureParameteri(bloomTextures[0], GL_TEXTURE_WRAP_S,
                        GL_CLAMP_TO_BORDER);
    glTextureParameteri(bloomTextures[0], GL_TEXTURE_WRAP_T,
                        GL_CLAMP_TO_BORDER);

    glTextureParameteri(bloomTextures[1], GL_TEXTURE_MIN_FILTER,
                        GL_LINEAR_MIPMAP_LINEAR);
    glTextureParameteri(bloomTextures[1], GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTextureParameteri(bloomTextures[1], GL_TEXTURE_WRAP_S,
                        GL_CLAMP_TO_BORDER);
    glTextureParameteri(bloomTextures[1], GL_TEXTURE_WRAP_T,
                        GL_CLAMP_TO_BORDER);

    glTextureParameteri(bloomTextures[2], GL_TEXTURE_MIN_FILTER,
                        GL_LINEAR_MIPMAP_LINEAR);
    glTextureParameteri(bloomTextures[2], GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTextureParameteri(bloomTextures[2], GL_TEXTURE_WRAP_S,
                        GL_CLAMP_TO_BORDER);
    glTextureParameteri(bloomTextures[2], GL_TEXTURE_WRAP_T,
                        GL_CLAMP_TO_BORDER);
...
void Renderer::LightPass(const World& world) {
  ...
  if (backgroundType != BackgroundType::None) {
    ...
    m_backgroundProgram->SetUniform(
        "miplevel",
        backgroundType == BackgroundType::Radiance ? backgroundMipLevel : 0);
...
void Renderer::ResolveMSAA() {
  ...
  glBlitNamedFramebuffer(m_msaaFB, m_resolveFB, 0, 0, m_framebufferSize.x,
                         m_framebufferSize.y, 0, 0, m_framebufferSize.x,
                         m_framebufferSize.y, GL_COLOR_BUFFER_BIT, GL_NEAREST);
...
void Renderer::Bloom() {
  ...
  // Prefilter
  glBindImageTexture(0, bloomTextures[0], 0, GL_FALSE, 0, GL_WRITE_ONLY,
                     GL_RGBA32F);
  ...
  m_bloomProgram->SetUniform(
      "u_params", glm::vec4(bloom.threshold, bloom.threshold - bloom.knee,
                            bloom.knee * 2.0f, 0.25f / bloom.knee));
  ...
  for (u32 i = 1; i < m_bloomPasses; ++i, lod += 1.0f) {
    ...
    glBindImageTexture(0, bloomTextures[1], i, GL_FALSE, 0, GL_WRITE_ONLY,
                       GL_RGBA32F);
    ...
    glBindImageTexture(0, bloomTextures[0], i, GL_FALSE, 0, GL_WRITE_ONLY,
                       GL_RGBA32F);
  ...
  m_bloomProgram->SetUniform("u_lod", lod);
  glBindImageTexture(0, bloomTextures[2], m_bloomPasses, GL_FALSE, 0,
  ...
  for (i32 mip = m_bloomPasses - 1; mip >= 0; mip--) {
    ...
    glBindImageTexture(0, bloomTextures[2], mip, GL_FALSE, 0, GL_WRITE_ONLY,
                       GL_RGBA32F);
    glBindTextureUnit(1, bloomTextures[0]);
...
void Renderer::Compose() {
  ...
  m_outputProgram->SetUniform("bloomAmount",
