...
extern std::vector<glm::vec3> PrecomputeDFG(u32 w,
                                            u32 h,
                                            u32 sampleCount);  // 128, 128, 512
...
void LoadEnvironment(const char* filename, Environment* env) {
  ...
  if (!glIsTexture(env->iblDFG)) {
    ...
    glTextureSubImage2D(env->iblDFG, 0, 0, 0, 128, 128, GL_RGB, GL_FLOAT,
                        PrecomputeDFG(128, 128, 1024).data());
  ...
  // glTextureStorage2D(equirectangularTexture, levels, GL_RGB32F, w, h);
  glTextureStorage2D(equirectangularTexture, log2f(beard::min(w, h)), GL_RGB32F,
                     w, h);
  glTextureSubImage2D(equirectangularTexture, 0, 0, 0, w, h, GL_RGB, GL_FLOAT,
                      data);
  glTextureParameteri(equirectangularTexture, GL_TEXTURE_WRAP_S,
                      GL_CLAMP_TO_EDGE);
  glTextureParameteri(equirectangularTexture, GL_TEXTURE_WRAP_T,
                      GL_CLAMP_TO_EDGE);
  ...
  if (!glIsTexture(env->envMap)) {
    ...
    glTextureStorage2D(env->envMap, log2f(cubemapSize), GL_RGBA32F, cubemapSize,
                       cubemapSize);
  ...
  if (!glIsTexture(env->radianceMap)) {
    ...
    glTextureStorage2D(env->radianceMap, 6, GL_RGBA32F, cubemapSize,
                       cubemapSize);
    ...
    glTextureParameteri(env->radianceMap, GL_TEXTURE_MIN_FILTER,
                        GL_LINEAR_MIPMAP_LINEAR);
    glTextureParameteri(env->radianceMap, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  }

  Program* prefilterEnvmapProgram =
      Program::GetProgramByName("prefilterEnvmap");
  ...
  for (u32 mip = 0; mip < mipLevels; ++mip, mipSize /= 2) {
    ...
    glBindImageTexture(1, env->radianceMap, mip, GL_FALSE, 0, GL_WRITE_ONLY,
                       GL_RGBA32F);
    prefilterEnvmapProgram->SetUniform("u_roughness", roughness);
    prefilterEnvmapProgram->SetUniform("u_mipSize",
                                       glm::vec2(mipSize, mipSize));
  ...
  if (!glIsTexture(env->irradianceMap)) {
    ...
    glTextureParameteri(env->irradianceMap, GL_TEXTURE_WRAP_S,
                        GL_CLAMP_TO_EDGE);
    glTextureParameteri(env->irradianceMap, GL_TEXTURE_WRAP_T,
                        GL_CLAMP_TO_EDGE);
    glTextureParameteri(env->irradianceMap, GL_TEXTURE_WRAP_R,
                        GL_CLAMP_TO_EDGE);
    glTextureParameteri(env->irradianceMap, GL_TEXTURE_MIN_FILTER,
                        GL_LINEAR_MIPMAP_LINEAR);
  ...
  glBindImageTexture(1, env->irradianceMap, 0, GL_TRUE, 0, GL_WRITE_ONLY,
                     GL_RGBA32F);
