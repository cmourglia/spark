...
inline glm::vec3 HemisphereImportanceSampleDGGX(const glm::vec2& u,
                                                const f32 a) {
...
glm::vec2 DFV(f32 NoV, f32 roughness, u32 sampleCount) {
  ...
  for (u32 i = 0; i < sampleCount; ++i) {
    ...
    if (NoL > 0) {
       ...
       * F(h) = f0*(1 - Fc) + f90*Fc
       *
       * f0 and f90 are known at runtime, but thankfully can be factored out,
       * allowing us to split the integral in two terms and store both terms
       * separately in a LUT.
       *
...
std::vector<glm::vec3> PrecomputeDFG(u32 w,
                                     u32 h,
                                     u32 sampleCount)  // 128, 128, 512
{
  ...
  for (u32 y = 0; y < h; ++y) {
    ...
    for (u32 x = 0; x < w; ++x) {
      ...
      // 	const f32 cosTheta = sqrtf((1.0f - h.y) / (1.0f + (m2 - 1.0f) *
      // h.y)); 	const f32 sinTheta = sqrtf(1.0f - cosTheta * cosTheta);
      ...
      // 		const f32 ndotlVisPDF = NoL * vis * (4.0f * vdoth /
      // ndoth); 		const f32 fresnel     = powf(1.0f -
      // vdoth, 5.0f);
