...
inline u32 CompileShader(const char* filename,
                         GLenum shaderType,
...
Program* Program::MakeRender(const char* name,
                             const char* vsfile,
                             const char* fsfile,
                             const StringArray& defines) {
...
Program* Program::MakeCompute(const char* name,
                              const char* csfile,
                              const StringArray& defines) {
  if (!g_programsByName.contains(name)) {
    Program program(name);
    ...
    program.m_shaders.push_back(
        {GL_COMPUTE_SHADER, cshader.c_str(),
         std::filesystem::last_write_time(cshader.c_str())});
    program.Build();
...
void Program::Build() {
  ...
  for (const auto& shader : m_shaders) {
    GLuint shaderID =
        CompileShader(shader.filename.data(), shader.type, m_defines);
...
void Program::GetUniformInfos() {
  ...
  if (uniformCount != 0) {
    ...
    for (GLint i = 0; i < uniformCount; ++i) {
      glGetActiveUniform(m_id, i, maxNameLength, &length, &count, &type,
                         uniformName);
