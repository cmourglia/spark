...
i32 main() {
  ...
  // &g_World)"); LoadScene(R"(resources/models/blender_probe/probe.glb)",
  // &g_World); LoadScene(R"(resources/models/3spheres.glb)", &g_World);
  // LoadScene(R"(external\glTF-Sample-Models\2.0\AnimatedCube\glTF\AnimatedCube.gltf)",
  // &g_World);

  // LoadScene(R"(external\glTF-Sample-Models\2.0\Box\glTF\Box.gltf)",
  // &g_World);
  LoadScene(R"(external\glTF-Sample-Models\2.0\Cube\glTF\Cube.gltf)", &g_World);
  // LoadScene(R"(external\glTF-Sample-Models\2.0\DamagedHelmet\glTF\DamagedHelmet.gltf,
  // &g_World)");
...
static void WheelCallback(GLFWwindow* window, f64 x, f64 y) {
  if (!movingCamera) {
    ...
    if (UI::IsCursorInViewport(mouseX, mouseY)) {
      ...
      const f32 multiplier = 2.5f * (g_Camera.distance - minDistance) /
